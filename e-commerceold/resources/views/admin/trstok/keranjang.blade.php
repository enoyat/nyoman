...
<div class="col-xs-4 col-sm-4 col-md-4 col-lg-12">
   ...
   <h2 class="title">Transaksi Stok</h2>

<a href="{{ route('atrstok') }}"><div id="viewData" class="btn btn-info">Refresh</div></a>
<a href="{{ route('atrstok.create') }}"><div id="viewData" class="btn btn-info">Tambah barang</div></a>
...
<form class="form-horizontal" method="post" action="{{ route('atrstok.incheckout') }}">
  @csrf
  <?php $i=1;
    $total=0;
    $totberat=0;
    ...
    <tr>
        <td> <?php echo $i;?></td>         
         <td width="10%">
          ...
          <td style="text-align: right;"><?php echo "Rp. ".number_format($fiturs->harga,0,',','.'); ?></td>
          <td style="text-align: right;"><?php echo $fiturs->qty; ?>            
          </td>
          <td style="text-align: right;"><?php $subtotal =$fiturs->qty*$fiturs->harga;
...
<div class="modal fade" id="modal_hapus<?php echo $id; ?>" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
    ...
    <form class="form-horizontal" method="post" action="{{ route('atrstok.hapuskeranjang') }}">
       @csrf
...
<div class="modal fade" id="modal_edit<?php echo $id; ?>" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
    ...
    <form class="form-horizontal" method="post" action="{{ route('atrstok.ubahqty') }}">
      @csrf
        <div class="modal-body">
            <div class="form-group">
              <label for="inputQty" class="col-sm-2 control-label">Qty:</label>
              <div class="col-sm-10">
                <input type="text" name="qty" id="qty" class="form-control" value="<?php echo $qty; ?>" required="required" title="">
              </div>
            </div>
            <div class="form-group">
              <label for="inputQty" class="col-sm-2 control-label">Harga:</label>
              <div class="col-sm-10">
                <input type="text" name="harga" id="harga" class="form-control" value="<?php echo $harga; ?>" required="required" title="">
              </div>
            </div>
