        ...
        @foreach ($dataorder as $key)
            ...
            <form class="form-horizontal" method="post" action="{{ route('checkout.order') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="kdorder" placeholder="Kode order"
                        value="{{ $key->kdorder }}   " readonly>
                </div>
                <div class="form-group">
                    <label>Tgl.Order</label>
                    <input type="text" class="form-control" id="kdorder" placeholder="Kode order"
                        value="{{ date('d-m-Y', strtotime($key->tglorder)) }}      " readonly>
                </div>
                <div class="form-group">
                    <label>Nama Penerima </label><i> (silahkan diganti jika nama penerima berbeda) </i>
                    <input type="text" name="penerima" id="inputpenerima" value="{{ $key->namamember }} "
                        required="required" title="" class="form-control">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="kdorder" placeholder="Kode order"
                        value="{{ 'Rp' . number_format($key->total, 0, ',', '.') }}  " readonly style="text-align:right;">
                </div>
                <div class="form-group">
                    <label>Sub Total Berat Produk (gr)</label>
                    <input type="text" name="berat" id="berat" size=10 value="<?php echo $totberat; ?>"
                        required="required" title="" readonly style="text-align: right;" class="form-control">
                </div>
                <div class="form-group">
                    <label>Metode Pengiriman</label>
                    <div class="filter__sort">
                        <select name="metodekirim" id="metodekirim" required="required" class="form-control">
                ...
                <div id="jasakurir">
                    ...
                    <div class="form-group">
                        ...
                        <label>Ongkir</label>
                        <input style="text-align: right;" type="text" name="biayaongkir" id="biayaongkir"
                            class="form-control" value="" required="required" title="" readonly="">
                    </div>
                ...
                <div class="form-group">
                    <label>Jumlah</label>
                    <input type="hidden" name="total" id="total" class="form-control" value="<?php echo $total; ?>"
                        required="required" title="">
                    <input type="text" name="totalview" id="totalview" class="form-control"
                        value="<?php echo 'Rp' . number_format($total, 0, ',', '.'); ?>" required="required" title=""
                        style="text-align:right; font-size: 15px;color:red;" readonly="">

                </div>

                <div class="form-group">
                    <label>Sub Total Pengiriman</label>
                    <input type="text" name="biayaongkirview" id="biayaongkirview" class="form-control"
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" name="totalorder" id="totalorder" class="form-control" value=""
                        required="required" title="" style="text-align:right;">
                    <input type="text" name="totalorderview" id="totalorderview" class="form-control" value=""
                        required="required" title="" style="text-align:right; font-size: 24px;color:red;"
                        readonly="">

                </div>

                <button type="submit" class="btn btn-danger" onclick="return confirm('Buat Pesanan ?')">BUAT
                    PESANAN</button>
...
?>
    <!-- ============ MODAL HAPUS  =============== -->
    <div class="modal fade" id="modal_hapus<?php echo $kdorder; ?>" tabindex="-1" role="dialog"
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        dataCity += '<option value=' + data[i].city_id + '>' + data[i].city_name +
                            '</option>';
                    }
        ...
        $('#kdkota').change(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        datakurir += '<option value=' + data[i].value + '>' + data[i].nama +
                            '</option>';
                    }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < cost.length; i++) {
                        dataCost += '<option value="' + cost[i].cost[0].value + '" data-nama="' + cost[
                                i].service + '">' + cost[i].service + ' - ' + cost[i].cost[0].value +
                            '</option>';
                    }
